...
function printToPrinter(printer, page) {
  ...
  return request(`${printer.server.host}/print-tickets`, postData(printer.server.api_key, body));
}
...
function printToReceipt(printer, page) {
  ...
  return request(`${printer.server.host}/print-receipts`, postData(printer.server.api_key, body));
}
